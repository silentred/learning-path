...
myLogger = logging.getLogger('v1188ys.importer.index_item')
def initSession():
...
def searchVideo(session,orig_id, video_type_id):
    try:
        ...
        myLogger.error('SearhVideo Error')
        raise e
    return video
...
def searchAndSaveIndexItem(session, rankObjs):
    ...
    for rank in rankObjs:
        try:
            ...
            myLogger.error('cannot find video whose orig_id=%s, video_type_id=%s ' % (rank['orig_id'], rank['video_type_id']))
            continue
        except Exception, e:
            raise e
